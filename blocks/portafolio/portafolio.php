                ...
                <div class="galleryContainer">                 
                    <div class="galleryThumbnailsContainer">
                        <div class="galleryThumbnails scrollpanel">
                            <?php
                            $rslidet = $database->query("SELECT idPr, galId, image FROM press_gal LEFT JOIN (SELECT idGal, pageId FROM galleries)galleries ON image_gal.galId=galleries.idGal WHERE pageId = '$bid'");
                    ...
                    <div class="galleryPreviewContainer">
                        <div class="galleryPreviewImage">  
                            ...
                            <?php
                            $rslider = $database->query("SELECT idPr, galId, image, caption FROM press_gal LEFT JOIN (SELECT idGal, pageId FROM galleries)galleries ON image_gal.galId=galleries.idGal WHERE pageId = '$bid'");
                            $rows = array();
                            while ($row = $rslider->fetch_array()) {
                                $rows[] = $row;
                            }
