...
* [#14620](https://dev.ckeditor.com/ticket/14620): Fixed: Setting both the `min-height` style for the `<body>` element and the `height` style for the `<html class="no-js" lang="en">` element breaks the [Auto Grow](http://ckeditor.com/addon/autogrow) plugin.
...
* The default class of captioned images has changed to `image` (was: `caption_en`). Please note that once edited in CKEditor 4.4+, all existing images of the `caption_en` class (`<figure class="caption">`) will be [filtered out](http://docs.ckeditor.com/#!/guide/dev_advanced_content_filter) unless the [`config.image2_captionedClass`](http://docs.ckeditor.com/#!/api/CKEDITOR.config-cfg-image2_captionedClass) option is set to `caption_en`. For backward compatibility (i.e. when upgrading), it is highly recommended to use this setting, which also helps prevent CSS conflicts, etc. This does not apply to new CKEditor integrations.
